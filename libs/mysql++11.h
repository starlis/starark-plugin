...
inline unsigned long castu32( const size_t& value ) {
    assert(value == static_cast<size_t>(static_cast<unsigned long>(value)));
...
//				if (!get_value(i, v)) throw mysqlpp_exception(mysqlpp_exception::empty_result);
        ...
        public:
            ...
            result query(const std::string& query_str) {
                ...
                int ret = mysql_real_query(my_conn, query_str.c_str(),castu32(query_str.length()));
            ...
            struct my_bind<std::string> : my_bind_base {
                ...
                virtual void pre_execute() override {
                    ...
                    if (data != nullptr) {
                        ...
                        bind->buffer_length = castu32(data->size());
                        bind->buffer_type = MYSQL_TYPE_STRING;
                        bind->is_null_value = false;
                        bind->length_value = castu32(data->size());
                    }
                    else {
            ...
            struct my_bind<optional<std::string>> : my_bind_base {
                ...
                virtual void pre_execute() override {
                    ...
                    if (data->has_value()) {
                        ...
                        bind->buffer_length = castu32(data->value().size());
                        bind->buffer_type = MYSQL_TYPE_STRING;
                        bind->is_null_value = false;
                ...
                virtual void pre_fetch() override {
                    ...
                    *data = std::string();
                    ...
                    bind->buffer_length = castu32(pdata.size());
                    bind->is_null_value = false;
                    bind->length = &bind->length_value;
                    bind->is_null = &bind->is_null_value;
                }
        ...
        public:
                    ...
                    : con(pcon), stmt(nullptr, [](MYSQL_STMT* stmt) { mysql_stmt_close(stmt); }), param_binds(0), result_binds(0)
            {
                ...
                if (mysql_stmt_prepare(stmt.get(), query.c_str(), castu32(query.size())))
